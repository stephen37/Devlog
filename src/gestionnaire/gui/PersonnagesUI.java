...
public class PersonnagesUI extends JDialog {
	...
	public PersonnagesUI(JFrame owner, Gestionnaire gesionnaire){
		super(owner);
		init();
		initListPersonnages();
		initPersonnagesIntoTable();
                gesionnaire = this.gestionnaire;
		pack();
	...
	private void initListPersonnages (){
		...
		String[] colName = { "Nom", "Point vie", "Point force", "Arme ou type" };
		tmodel = new DefaultTableModel() {

		};
		// /!\ Have to set the column Identifiers... or maybe with a DefaultColumnTableModel would work!
		...
		TableModelListener jtable_listener = new TableModelListener(){
			/* TODO: this is crappy since it is called everytime the Jtable is changed. 
			 * Even when initializing it, it is fired for non editable cells.
			 *  Find a way of adding a Listener on specifics Cells */
	...
	private void initPersonnagesIntoTable(){
		...
		for(Personnage perso: gestionnaire.getPersonnages()){
			Object[] test = { perso, perso.getNom(), perso.getVie(), perso.getForce()};
			tmodel.addRow( test );
		}
	}
